...
<body
    ...
    <div
        class="max-w-5xl w-full bg-white border border-outline-variant/60 rounded-3xl card-elevation overflow-hidden grid grid-cols-1 lg:grid-cols-12 min-h-[660px]">
        ...
        <div
            ...
            <div class="my-8 lg:my-auto z-10 space-y-6">
                <div class="space-y-2">
                    <span
                        class="text-[11px] font-extrabold px-3 py-1 bg-secondary/20 text-secondary-container rounded-full inline-block border border-secondary/30">
                        🚀 حساب مجاني بالكامل
                    </span>
                    <h2 class="text-2xl lg:text-3xl font-black leading-tight text-white">
                        ابدأ رحلة التخطيط <br>
                        <span
                            class="text-transparent bg-clip-text bg-gradient-to-l from-indigo-200 via-white to-indigo-300">الفائق
                            بذكاء الـ AI</span>
                    </h2>
                    <p class="text-indigo-100/80 text-xs sm:text-sm leading-relaxed font-medium max-w-sm">
                        أنشئ حسابك الآن لتبدأ بتنظيم مهامك يدوياً، واستعن بـ AI Agile Architect لبناء وتقسيم مشروعك
                        البرمجي القادم في ثوانٍ.
                    </p>
                </div>

                <div
                    class="bg-white/10 backdrop-blur-md border border-white/15 rounded-2xl p-4 space-y-3 shadow-inner max-w-sm text-xs font-bold text-indigo-100">
                    <div class="flex items-center gap-2.5">
                        <span class="material-symbols-outlined text-secondary-container text-[18px]">verified</span>
                        <span>إدارة مهام كاملة (Kanban, Backlogs & Sprints)</span>
                    </div>
                    <div class="flex items-center gap-2.5">
                        <span class="material-symbols-outlined text-secondary-container text-[18px]">verified</span>
                        <span>تفكيك فوري للأفكار المعقدة بالذكاء الاصطناعي</span>
                    </div>
                    <div class="flex items-center gap-2.5">
                        <span class="material-symbols-outlined text-secondary-container text-[18px]">verified</span>
                        <span>توليد تلقائي لنقاط الجهد ومعايير القبول</span>
                    </div>
                </div>
            </div>
            ...
            <div
                ...
                <span class="flex items-center gap-1"><span
                        class="w-1.5 h-1.5 rounded-full bg-emerald-400 inline-block"></span> الجيل الجديد 2.0</span>
            </div>
        </div>
        ...
        <div class="lg:col-span-7 p-8 lg:p-12 flex flex-col justify-center bg-white relative">
            ...
            <div class="max-w-md w-full mx-auto space-y-5">
                ...
                <div>
                    <h1 class="text-2xl sm:text-3xl font-black text-on-surface mb-1.5">إنشاء حساب جديد 🚀</h1>
                    <p class="text-on-surface-variant text-xs sm:text-sm font-medium">انضم إلينا مجاناً واختبر قوة
                        الإنتاجية المدعومة بالـ AI</p>
                </div>

                <form action="{{ route('register') }}" method="POST" class="space-y-4">
                    @csrf <div>
                        <label class="block text-xs font-extrabold text-on-surface mb-1.5">الاسم الكامل</label>
                        <div class="relative">
                            <span
                                class="material-symbols-outlined absolute right-3.5 top-1/2 -translate-y-1/2 text-on-surface-variant text-[20px]">person</span>
                            <input type="text" name="name" value="{{ old('name') }}" required autofocus
                                placeholder="مثال: محمد جودة"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-extrabold text-on-surface mb-1.5">كلمة المرور</label>
                            <div class="relative">
                                <span
                                    class="material-symbols-outlined absolute right-3 top-1/2 -translate-y-1/2 text-on-surface-variant text-[18px]">lock</span>
                                <input :type="showPassword ? 'text' : 'password'" name="password" required
                                    autocomplete="new-password" placeholder="••••••••"
                                    class="w-full pr-9 pl-9 py-3 bg-surface-container/40 border {{ $errors->has('password') ? 'border-error ring-1 ring-error/20' : 'border-outline-variant/80' }} rounded-2xl text-xs text-on-surface focus:outline-none focus:border-primary focus:bg-white transition-all font-medium font-geist">

                                <button type="button" @click="showPassword = !showPassword"
                                    class="absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant hover:text-on-surface focus:outline-none p-1 transition-colors">
                        ...
                        <div>
                            <label class="block text-xs font-extrabold text-on-surface mb-1.5">تأكيد كلمة المرور</label>
                            <div class="relative">
                                <span
                                    class="material-symbols-outlined absolute right-3 top-1/2 -translate-y-1/2 text-on-surface-variant text-[18px]">gpp_good</span>
                                <input :type="showPassword ? 'text' : 'password'" name="password_confirmation" required
                                    placeholder="••••••••"
                                    class="w-full pr-9 pl-4 py-3 bg-surface-container/40 border border-outline-variant/80 rounded-2xl text-xs text-on-surface focus:outline-none focus:border-primary focus:bg-white transition-all font-medium font-geist">
                            </div>
                        </div>
                    </div>
                    @error('password')
                    ...
                    <button type="submit"
                        class="w-full mt-3 bg-gradient-to-r from-primary to-indigo-600 hover:from-indigo-600 hover:to-primary text-white font-bold py-3.5 px-4 rounded-2xl shadow-lg shadow-primary/25 hover:shadow-primary/40 transform active:scale-95 transition-all duration-200 flex items-center justify-center gap-2 group">
                        <span>إنشاء الحساب والبدء فوراً</span>
                        <span
                            class="material-symbols-outlined text-[18px] group-hover:rotate-12 transition-transform">auto_awesome</span>
                    </button>
                </form>
                ...
                <div class="relative my-5">
                    ...
                    <div class="relative flex justify-center text-[11px] uppercase"><span
                            class="bg-white px-3 text-on-surface-variant font-bold">أو التسجيل السريع بواسطة</span>
                    </div>
                </div>

                <div class="grid grid-cols-2 gap-3">
                    <button type="button"
                        class="flex items-center justify-center gap-2 py-2.5 px-4 border border-outline-variant/80 rounded-2xl hover:bg-surface-container/50 transition-colors font-bold text-xs text-on-surface">
